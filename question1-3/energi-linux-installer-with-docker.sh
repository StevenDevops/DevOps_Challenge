...
_add_nrgstaker () {
  ...
  then
      ...
      then
        REPLY=''
      read -t 10 -p "Timeout 10 sec! Did you write down the username and password? y/[n]: "
      REPLY=${REPLY,,} # tolower
      if [[ "${REPLY}" == "n" ]] || [[ -z "${REPLY}" ]]
      then
        echo
...
_install_energi () {
  ...
  then
    ...
    rm -rf ${ENERGI_EXE}-${GIT_VERSION_TAG}-linux-${OSARCH} SHA256SUMS
  else
    mv ${ENERGI_EXE}-${GIT_VERSION_TAG}-linux-${OSARCH} ${ENERGI_EXE}
  fi
  rm ${ENERGI_EXE}-${GIT_VERSION_TAG}-linux-${OSARCH}.tgz SHA256SUMS
